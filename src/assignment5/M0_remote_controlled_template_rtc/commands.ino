...
void showRoomState() {
  ...
  display.display();
  Serial.print("<<SEND:"); Serial.print(rtc.getEpoch());Serial.print(":");Serial.print(count);Serial.println(":>>");
}
...
void runTaskControlled(uint8_t cmd, uint8_t param, boolean repeat){
  ...
  if (cmd==11) {
    showRoomState();
    if(repeat){
      insertTask(getTimeInSeconds()+showPeriod, cmd, 0);
    }
  }
  if (cmd==12) {
    do_send(&sendjob);
